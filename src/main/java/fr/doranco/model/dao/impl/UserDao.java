package fr.doranco.model.dao.impl;
...
import fr.doranco.model.dao.interfaces.IAdresseDao;
...
public class UserDao implements IUserDao {
	...
	public User addUser(User user) throws Exception {
		...
		try {
			...
			String requete = "INSERT INTO user(nom, prenom, date_naissance,email,password,telephone,profil) VALUES(?,?,?,?,?,?,?)";
			...
			ps.setBytes(5, Cryptage.encrypt(user.getPassword()));
	...
	public User getUserById(int id) throws Exception {
		...
		try {
			...
			if (rs != null && rs.next()) {
				...
				user.setPassword(Cryptage.decrypt(rs.getBlob("password")));
				user.setTelephone(rs.getString("telephone"));
				user.setProfil(Profil.values()[rs.getInt("profil")]);
	...
	public void updateUser(User user) throws Exception {
		...
		try {
			connection = DataBaseConnection.getConnection();
			String requete = "UPDATE `user` SET `nom`=?,`prenom`=?,`date_naissance`=?,"
					+ "`email`=?,`password`=?,`telephone`=? ,`profil`=? "
					+ "WHERE id = ?";
			ps = connection.prepareStatement(requete);
			ps.setString(1, user.getNom());
			ps.setString(2, user.getPrenom());
	...
	public User getUserByEmail(String email) throws Exception {
		...
		try {
			...
			List<Adresse> adresses = adresseDao.getAdresses(user.getId());
			for (Adresse adresse : adresses) {
				user.getAdresses().add(adresse);
			}
